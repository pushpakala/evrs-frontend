...
describe('DataTable.vue', () => {
  describe('Rendering tests', () => {
    ...
    it('should render the th class for each column', () => {
      const wrapper = mount(DataTable, { propsData: mock.headerColumnsCss })

      const ths = wrapper.findAll('.v-datatable-light thead tr th')
      expect(ths).toHaveLength(3)

      const firstThClass = ths.at(0).find('.header-cell-0')
      expect(firstThClass.exists()).toBe(true)

      const secondThClass = ths.at(1).find('.header-cell-1')
      expect(secondThClass.exists()).toBe(true)

      const thirdThClass = ths.at(2).find('.header-cell-2')
      expect(thirdThClass.exists()).toBe(true)
    })

    it('should only render the tr class for each line when there is a class in the respective index of rowsCss attribute', () => {
      const wrapper = mount(DataTable, { propsData: mock.rowsCss })

      const trs = wrapper.findAll('.v-datatable-light tbody tr.row-1')
    ...
    it('should render the tbody td class for each column using the column index to create the class name', () => {
      ...
      const tds = wrapper.findAll('.v-datatable-light tbody tr.row-1 td')
      expect(tds).toHaveLength(3)
