  ...
  <table :class="['v-datatable-light', css.table]">
    ...
    <tbody :class="css.tbody" :style="tbodyStyle">
      ...
      <template v-else-if="data.length">
        <tr v-for="(item, index) in data" :key="index" :class="getRowClass(index, css.tbodyTr)">
          <td
            v-for="key in headers"
            :key="`${index}-${key.name}`"
            :class="[css.tbodyTd, `column-${index}`]"
...
export default {
  ...
  methods: {
    ...
    getHeaderCellClass: function (columnIndex, extraClass) {
      if (this.headerrowsCss.length > columnIndex) {
        return `${this.headerrowsCss[columnIndex]} ${extraClass}`
      }
      return extraClass
    },

    getRowClass: function (columnIndex, extraClass) {
      if (this.rowsCss.length > columnIndex) {
        return `${this.rowsCss[columnIndex]} ${extraClass || ''}`
      }
      return extraClass
    }
  }
