    ...
    <thead :class="css.thead" :style="theadStyle">
      <tr :class="css.theadTr">
        <th
          ...
          >
          <!-- header free text -->
          <div v-if="!isFieldSpecial(item.name) && !item.customHeader" :class="getHeaderColumnClass(columnIndex, css.thWrapper)" @click="orderBy(item.name)">
            {{ item.label }}
...
export default {
  ...
  methods: {
    ...
    getHeaderColumnClass: function (columnIndex, extraClass) {
      if (this.headerColumnsCss.length > columnIndex) {
        return `${this.headerColumnsCss[columnIndex]} ${extraClass}`
      }
      return extraClass
    },
