  ...
  <table :class="['v-datatable-light', css.table]">
    <thead :class="css.thead" :style="theadStyle">
      <tr :class="css.theadTr">
        <th
          ...
          >
          <!-- header free text -->
          <div v-if="!isFieldSpecial(item.name) && !item.customHeader" :class="getHeaderCellClass(columnIndex, css.thWrapper)" @click="orderBy(item.name)">
            {{ item.label }}
    ...
    <tbody :class="css.tbody" :style="tbodyStyle">
      ...
      <template v-else-if="data.length">
        <tr v-for="(item, index) in data" :key="index" :class="getCellClass(index, css.tbodyTr)">
          <td
...
export default {
  ...
  methods: {
    ...
    getHeaderCellClass: function (columnIndex, extraClass) {
      if (this.headerCellsCss.length > columnIndex) {
        return `${this.headerCellsCss[columnIndex]} ${extraClass}`
      }
      return extraClass
    },

    getCellClass: function (columnIndex, extraClass) {
      if (this.cellsCss.length > columnIndex) {
        console.log(this.cellsCss, columnIndex, this.cellsCss[columnIndex])
        return `${this.cellsCss[columnIndex]} ${extraClass || ''}`
      }
      return extraClass
    }
  }
